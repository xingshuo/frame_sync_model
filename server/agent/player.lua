...
function Player:enterwar()
	local mt = skynet_call("SCENE_MGR", "enter_player", self.m_GameModeID, self.m_ID, self:PackData())
...
function Player:leavewar()
	if self.m_SceneAgent then
		skynet_send(self.m_SceneAgent, "leave_player", self.m_GameID, self.m_ID)
	end
end
...
function Player:calc_rtt()
	...
	self.m_RTT = math.floor(self.m_RTT)
	if math.abs(self.m_RTT - self.m_OldRTT) >= 5 then
		self.m_OldRTT = self.m_RTT
		skynet_send(self.m_SceneAgent, "sync_data", self.m_GameID, self.m_ID, {rtt = self.m_RTT})
	end
...
function Player:sync_ctrl(data)
	if self.m_SceneAgent then
		skynet_send(self.m_SceneAgent, "sync_ctrl", self.m_GameID, self.m_ID, data)
	end
end
